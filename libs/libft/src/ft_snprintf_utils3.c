...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/02 11:41:29 by blucken           #+#    #+#             */
/*   Updated: 2024/12/02 11:41:32 by blucken          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	handle_padding(char *str, size_t size, const char *src, t_format fmt)
{
	...
	if ((size_t)fmt.width > len)
	{
		...
		if (fmt.zero_pad && !fmt.left_align)
			...
			pad_char = ' ';
		if (!fmt.left_align)
			pad.pos += write_padding(str, pad, pad_char, padding);
		pad.pos += write_str(str, size, pad.pos, src);
		if (fmt.left_align)
			pad.pos += write_padding(str, pad, ' ', padding);
		return (fmt.width);
	}
	return (write_str(str, size, pad.pos, src));
}
